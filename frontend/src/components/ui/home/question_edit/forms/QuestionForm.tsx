...
import { getTopics, patchQuestion, postQuestion } from '@/services/questionServiceApi';
...
export default function QuestionForm({ mode, initialData }: QuestionFormProps) {
  ...
  const [topics, setTopics] = useState<string[]>(initialData?.topics.map(t => t.slug) || []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!difficulty.trim()) return alert('Please select a difficulty level.');
    if (topics.length === 0) return alert('Question needs to have at least one topic.');
    if (!body.trim()) return alert('Question body cannot be empty.');
    ...
    if (mode === 'edit' && initialData) {
      const payload = Object.entries(newData).reduce<Record<string, unknown>>((acc, [key, value]) => {
  ...
  return (
    <form onSubmit={handleSubmit} >
      <div className='grid grid-cols-1 gap-y-5'>
        ...
        <AttachmentsSection id={initialData?.id} attachments={attachments} setAttachments={setAttachments} />
