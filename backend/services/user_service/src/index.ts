...
const signupSchema = z.object({
    ...
    username: z.string()
        ...
        .regex(/^[a-z0-9_]+$/, { message: "Username can only contain lowercase letters, numbers, and underscores (_)." }),
});
...
const updateProfileSchema = z.object({
    username: z.string()
        ...
        .regex(/^[a-z0-9_]+$/, { message: "Username can only contain lowercase letters, numbers, and underscores (_)." })
        .optional(),
    bio: z.string().max(150, "Bio cannot exceed 150 characters.").optional().nullable(),
...
const updatePasswordSchema = z.object({
    ...
    newPassword: z.string() // same rules as sign up pw
        .min(8, { message: "Password must be at least 8 characters long." })
        .regex(/[A-Z]/, { message: "Password must contain at least one uppercase letter." })
        .regex(/[a-z]/, { message: "Password must contain at least one lowercase letter." })
        .regex(/[0-9]/, { message: "Password must contain at least one number." })
...
app.put('/api/users/me/profile', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
...
app.put('/api/users/me/email', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.status(200).json({ message: "Email updated successfully. Please use it for your next login." });
...
app.put('/api/users/me/password', authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
