...
export class MessageReceiver {
  ...
  public async listenForMessagesWithType(
    message_types: MESSAGE_TYPES[],
    message_handler: (
      routingKey: MESSAGE_TYPES,
      content: string,
    ) => Promise<any>,
  ): Promise<void> {
    if (!this.channel) {
    ...
    this.channel.consume(
      ...
      async (msg: ConsumeMessage | null) => {
        ...
        const messageType: MESSAGE_TYPES = Object.values(
          MESSAGE_TYPES,
        ).includes(messageTypeStr as MESSAGE_TYPES)
          ? (messageTypeStr as MESSAGE_TYPES)
          : MESSAGE_TYPES.Invalid;
        await message_handler(messageType, msg!.content.toString());
